...
static void process_mult_command(conn *c, token_t *tokens, const size_t ntokens) {
    char temp[INCR_MAX_STORAGE_LEN];
    uint64_t delta;
    char *key;
    size_t nkey;

    out_string(c, "Start mult command");
    ...
    switch(mult_delta(c->thread, key, nkey, delta, temp, NULL)){
        case 0: {
            out_string(c, temp);
        } break;
        case 6: {
            out_string(c, "integer overflow");
        } break;
        case NON_NUMERIC:
            out_string(c, "CLIENT_ERROR cannot increment or decrement non-numeric value");
        break;
...
void process_command_ascii(conn *c, char *command) {
    ...
    if (first == 'm' && tokens[COMMAND_TOKEN].length == 2) {
        ...
        WANT_TOKENS(ntokens, 4, 4);
        process_mult_command(c, tokens, ntokens);
