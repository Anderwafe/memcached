...
int do_mult_delta(LIBEVENT_THREAD *t, const char *key, const size_t nkey,
        const int64_t delta,
        char *buf, uint64_t *cas,
        const uint32_t hv,
        item **it_ret) {
    ...
    if (cas != NULL && *cas != 0 && ITEM_get_cas(it) != *cas) {
        do_item_remove(it);
        return DELTA_ITEM_CAS_MISMATCH; // 4
    }
